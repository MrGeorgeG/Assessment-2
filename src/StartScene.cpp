...
void StartScene::m_updateUI()
{
	...
	std::string windowString = "Physical Settings ";
	...
	if (ImGui::SliderFloat("Shooting Angle", &m_angle, 0.0f, 90.0f, "%.1f"))
	{
	...
	if (ImGui::SliderFloat("Force", &m_force, 0.0f, 90.0f, "%.1f"))
	{

	}
	if (ImGui::SliderFloat("Mass(kg)", &m_mass, 0.0f, 200.0f, "%.1f"))
	{
...
{
	//velocity components 
	m_finalPosition = m_pShip->getPosition() + (m_velocity * m_time);
	/*m_velocityX = (m_velocity * m_PPM) * cos(m_angle*Deg2Rad);
	m_velocityY = (m_velocity * m_PPM) * -sin(m_angle*Deg2Rad);*/
	m_velocityX = m_gravity * m_time;
	m_velocityY =  m_force / m_mass * m_time;
	...
	if (m_finalPosition.y > 500.0f)
	{
		float windDisplacement = m_force * 2 * (m_velocity * m_velocity) * (glm::sin(m_angle * Deg2Rad) * glm::sin(m_angle * Deg2Rad)) / (m_mass * (m_gravity * m_gravity));
		m_finalPosition.y = 500;
		m_finalPosition.x = ((((m_velocity * m_velocity) * glm::sin((m_angle * Deg2Rad) * 2)) / m_gravity) + windDisplacement + 50.0f);
